...
export function AuthProvider({ children }) {
  ...
  const checkRateLimit = useCallback((identifier) => {
    ...
    if (record) {
      if (
        ...
        now - record.firstAttempt < LOCKOUT_DURATION
      ) {
        const remaining = Math.ceil(
          (LOCKOUT_DURATION - (now - record.firstAttempt)) / 60000,
        );
        throw new Error(
          `Too many login attempts. Please try again in ${remaining} minute(s).`,
        );
      }
  ...
  const login = useCallback(
    async (email, password) => {
        ...
        try {
          ...
          if (u) {
            ...
            try {
              ...
              /* ignore fallback admin profile fetch check */
            }
      ...
      addToast("Login requires Supabase to be configured", "error");
    },
  ...
  const register = useCallback(
    async (firstName, lastName, email, password) => {
      ...
      if (dataService.isConfigured()) {
        try {
          const { user: u } = await dataService.signUp(safeEmail, password, {
          ...
          if (u) {
            ...
            addToast(
              `Account created for ${safeFirst} ${safeLast}! Welcome to Aisha.`,
              "success",
            );
            navigate("/");
            return;
      ...
      addToast("Registration requires Supabase to be configured", "error");
    },
